...
export function findLoreLinks(source:string){const out:string[]=[];for(const m of source.matchAll(/@([A-Za-z][A-Za-z0-9 '\\-]{0,60})/g)){let name=m[1].trim().replace(/\\s+(?:and|or|but)$/i,'').trim();if(name)out.push(name)}return out}
