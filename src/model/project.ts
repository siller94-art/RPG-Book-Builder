...
export function createImageBlock(src='',alt='Artwork'):DocumentBlock{return{id:uid(),kind:'image',title:alt,src,alt,fit:'contain',opacity:1,width:100,position:'center',layer:'inline'}}
export function createStatblock(template:StatblockKind='monster'):DocumentBlock{
 ...
 };return{id:uid(),kind:'statblock',template,title:template==='spell'?'New Spell':template==='item'?'New Item':template==='encounter'?'New Encounter':template==='trap'?'New Trap':template==='vehicle'?'New Vehicle':template==='custom'?'Custom Block':'New Creature',body:'Add rules and details here.',fields:presets[template]}}
...
export function normalizeProject(value:unknown):Project{if(!value||typeof value!=='object')throw new Error('Invalid project');const p=value as Partial<Project>;if(p.schemaVersion!==1||!Array.isArray(p.pages)||!p.pages.length)throw new Error('Unsupported or damaged project file');for(const page of p.pages)for(const block of page.blocks){if(block.kind==='statblock'&&!block.fields)block.fields=(createStatblock(block.template) as any).fields;if(block.kind==='image'){block.width??=100;block.position??='center';block.layer??='inline';block.opacity??=1;block.fit??='contain'}}return p as Project}
