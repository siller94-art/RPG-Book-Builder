...
export function normalizeProject(value:unknown):Project{if(!value||typeof value!=='object')throw new Error('Invalid project');const p=value as Partial<Project>;if(p.schemaVersion!==1||!Array.isArray(p.pages)||!p.pages.length)throw new Error('Unsupported or damaged project file');p.settings??={pageSize:'letter',orientation:'portrait',themeId:'parchment'};p.settings.orientation??='portrait';for(const page of p.pages)for(const block of page.blocks){block.width??=block.kind==='statblock'?62:100;block.spacing??=18;if(block.kind==='statblock'&&!block.fields)block.fields=createStatblock(block.template).fields;if(block.kind==='image'){block.width??=100;block.position??='center';block.layer??='inline';block.opacity??=1;block.fit??='contain'}}return p as Project}
